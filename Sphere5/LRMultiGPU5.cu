...
#define LRLEVEL 2
#define LRX0 10.25f    //minimum x coord of LR
#define XLRDIM 40    //number of nodes in x
#define LRY0 40.25f
#define YLRDIM 40
#define LRZ0 -0.75f
#define ZLRDIM 86
...
//#define LRLEVEL 4
//#define LRX0 224.125f    //minimum x coord of LR
//#define XLRDIM 768    //number of nodes in x
//#define LRY0 300.125f
//#define YLRDIM 1280
//#define LRZ0 -0.875f
//#define ZLRDIM 32
//#define ORDER 2 //order of accuracy of interpolation

#define RE 250.f//2000.f//100.f;
#define UMAX 0.08f
    ...
    if(im == 1 || im ==10){//BB
        ...
        if(im == 200)//south inlet
        {
            for(int i = 0; i<19; i++)
                   f[i ]= hA[buff_mem(i ,x,y+1,pitch)];
            //South_Extrap(f,UMAX);
            float u_in = UMAX;//PoisProf3D(x,(GPU+1)*(zInner+2)-1);
            South_Extrap(f,u_in);
        }
        if(im == 25)
    ...
    if(im == 1 || im ==10){//BB
        ...
        if(im == 200)//south inlet
        {
            for(int i = 0; i<19; i++)
                   f[i ]= gA[buff_mem(i ,x,y+1,pitch)];
            //South_Extrap(f,UMAX);
            float u_in = UMAX;//PoisProf3D(x,GPU*(zInner+2));
            South_Extrap(f,u_in);
        }
        if(im == 25)
    ...
    if(im == 1 || im ==10){//BB
        ...
        if(im == 200)//south inlet
        {
            for(int i = 0; i<19; i++)
                   f[i ]= fA[f_mem(i ,x,y+1,z,pitch,zInner)];
            //South_Extrap(f,UMAX);
            float u_in = UMAX;//PoisProf3D(x,GPU*(zInner+2)+1+z);
            South_Extrap(f,u_in);
        }
        if(im == 25)
