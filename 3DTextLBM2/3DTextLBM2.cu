...
__device__ int f_mem(int x, int y, int z, size_t pitch, int height, int depth)
{
//	if (x<0 || x>pitch || y<0 || y>height || z<0 || z>depth) return 0;
//	else
		return (x+y*pitch+z*height*pitch);
}
...
__global__ void test(float* f0out, float* f1out, float* f2out,
							float* f3out, float* f4out, float* f5out,
							float* f6out, float* f7out, float* f8out, float* f9out,
//							float* f10out, float* f11out, float* f12out,
//							float* f13out, float* f14out, float* f15out, float* f16out,
//							float* f17out, float* f18out,
							float* f2in, float* f4in, float* f9in, //float* f11in,
//							float* f13in, float* f14in, float* f16in, float* f18in,
...
//		f0 = f0out[j];
		f2 =  f2in[j];//tex2D(texRef_f4A,x  ,(y+1)+height*(z));
		f4 =  f4in[j];
		f9 =  f9in[j];
//		f11= f11in[x+y*pitch+z*height*pitch];
//		f13= f13in[x+y*pitch+z*height*pitch];
//		f14= f14in[x+y*pitch+(z)*height*pitch];
//		f16= f16in[x+y*pitch+z*height*pitch];
//		f18= f18in[x+y*pitch+z*height*pitch];
...
//		f2 =  f2in[x+(y-1)*pitch+z*height*pitch];//tex2D(texRef_f4A,x  ,(y+1)+height*(z));
//		f4 =  f4in[x+(y+1)*pitch+z*height*pitch];
//		f14= f14in[x+y*pitch+(z+1)*height*pitch];
//		f18= f18in[x+dmin(y+1,height)*pitch+dmin(z+1,depth)*height*pitch];
//		f16= f16in[x+dmax(y-1)*pitch+dmin(z+1,depth)*height*pitch];
//		f9 =  f9in[x+y*pitch+(z-1)*height*pitch];
//		f11= f11in[x+dmax(y-1)*pitch+dmax(z-1)*height*pitch];
//		f13= f13in[x+dmin(y+1,height)*pitch+dmax(z-1)*height*pitch];
////		f1 = tex2D(texRef_f1A ,x-1,y  +height*(z));
////		f3 = tex2D(texRef_f3A ,x+1,y  +height*(z));
////		f5 = tex2D(texRef_f5A ,x-1,y-1+height*(z));
////		f6 = tex2D(texRef_f6A ,x+1,y-1+height*(z));
...
__global__ void mrt_d_hybAB(float* f0out, float* f1out, float* f2out,
							float* f3out, float* f4out, float* f5out,
							float* f6out, float* f7out, float* f8out, float* f9out,
							float* f10out, float* f11out, float* f12out,
							float* f13out, float* f14out, float* f15out, float* f16out,
...
//		f2 =  f4in[f_mem(x,dmin(y+1,height),z,pitch,height,depth)];//tex2D(texRef_f4B,x  ,(y+1)+height*(z));
//		f4 =  f2in[f_mem(x,dmax(y-1),z,pitch,height,depth)];
//		f9 = f14in[f_mem(x,y,dmin(z+1,depth),pitch,height,depth)];
//		f14=  f9in[f_mem(x,y,dmax(z-1),pitch,height,depth)];
//		f11= f18in[f_mem(x,dmin(y+1,height),dmin(z+1,depth),pitch,height,depth)];
//		f18= f11in[f_mem(x,dmax(y-1),dmax(z-1),pitch,height,depth)];
//		f13= f16in[f_mem(x,dmax(y-1),dmin(z+1,depth),pitch,height,depth)];
//		f16= f13in[f_mem(x,dmin(y+1,height),dmax(z-1),pitch,height,depth)];

		f2 =  f4in[x+dmin(y+1,height)*pitch+z*height*pitch];//tex2D(texRef_f4B,x  ,(y+1)+height*(z));
		f4 =  f2in[x+dmax(y-1)*pitch+z*height*pitch];
		f9 = f14in[x+y*pitch+dmin(z+1,depth)*height*pitch];
		f14=  f9in[x+y*pitch+dmax(z-1)*height*pitch];
		f11= f18in[x+dmin(y+1,height)*pitch+dmin(z+1,depth)*height*pitch];
		f18= f11in[x+dmax(y-1)*pitch+dmax(z-1)*height*pitch];
		f13= f16in[x+dmax(y-1)*pitch+dmin(z+1,depth)*height*pitch];
		f16= f13in[x+dmin(y+1,height)*pitch+dmax(z-1)*height*pitch];
...
//		f0out[j] = f0out[j];
//		f2out[j] =  f4in[x+(y+1)*pitch+z*height*pitch];//tex2D(texRef_f4B,x  ,(y+1)+height*(z));
//		f4out[j] =  f2in[x+(y-1)*pitch+z*height*pitch];
//		f9out[j] = f14in[x+y*pitch+(z+1)*height*pitch];
//		f14out[j]=  f9in[x+y*pitch+(z-1)*height*pitch];
//		f11out[j]= f18in[x+dmin(y+1,height)*pitch+dmin(z+1,depth)*height*pitch];
//		f18out[j]= f11in[x+dmax(y-1)*pitch+dmax(z-1)*height*pitch];
//		f13out[j]= f16in[x+dmax(y-1)*pitch+dmin(z+1,depth)*height*pitch];
//		f16out[j]= f13in[x+dmin(y+1,height)*pitch+dmax(z-1)*height*pitch];
...
__global__ void mrt_d_hybBA(float* f0out, float* f1out, float* f2out,
							float* f3out, float* f4out, float* f5out,
							float* f6out, float* f7out, float* f8out, float* f9out,
							float* f10out, float* f11out, float* f12out,
							float* f13out, float* f14out, float* f15out, float* f16out,
...
//										image_d,omega,uMax,xDim,yDim,zDim,pitch_elements);
//
...
//										image_d,omega,uMax,xDim,yDim,zDim,pitch_elements);
		...
		test<<<grid, threads>>>(f0_dA,f1_dA,f2_dA,f3_dA,f4_dA,f5_dA,f6_dA,f7_dA,f8_dA,f9_dA,
//										f10_dA, f11_dA, f12_dA, f13_dA, f14_dA, f15_dA, f16_dA, f17_dA, f18_dA,
										f2_dB,f4_dB,f9_dB,
//										f11_dB, f13_dB, f14_dB, f16_dB, f18_dB,
										image_d,omega,uMax,xDim,yDim,zDim,pitch_elements);
