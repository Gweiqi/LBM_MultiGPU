...
#define DYNY1 100
#define DYNY2 1
#define KP 0.05f //p-control constant
#define PRERUN 1000 //pre-run duration
#define HEIGHT 200
#define INCOMP 0

#define OBSTR1 10.f
#define OBSTX1 50.5f
#define OBSTY1 140.5f
#define OBSTZ1 32.5f
...
inline __device__ void mrt_collide(float* f, float omega, float dpdy)
{
	...
	feq[0 ]=(0.3333333333f*rho*(1.f-1.5f*usqr));
	feq[1 ]=(0.0555555556f*rho*(1.f+3.0f*u+4.5f*u*u-1.5f*usqr));                
	feq[2 ]=(0.0555555556f*rho*(1.f+3.0f*v+4.5f*v*v-1.5f*usqr));
	feq[3 ]=(0.0555555556f*rho*(1.f-3.0f*u+4.5f*u*u-1.5f*usqr));
	feq[4 ]=(0.0555555556f*rho*(1.f-3.0f*v+4.5f*v*v-1.5f*usqr));
	feq[5 ]=(0.0277777778f*rho*(1.f+3.0f*(u+v)+4.5f*(u+v)*(u+v)-1.5f*usqr));
	feq[6 ]=(0.0277777778f*rho*(1.f+3.0f*(-u+v)+4.5f*(-u+v)*(-u+v)-1.5f*usqr));
	feq[7 ]=(0.0277777778f*rho*(1.f+3.0f*(-u-v)+4.5f*(-u-v)*(-u-v)-1.5f*usqr));
	feq[8 ]=(0.0277777778f*rho*(1.f+3.0f*(u-v)+4.5f*(u-v)*(u-v)-1.5f*usqr));
	feq[9 ]=(0.0555555556f*rho*(1.f+3.0f*w+4.5f*w*w-1.5f*usqr));
	feq[10]=(0.0277777778f*rho*(1.f+3.0f*(u+w)+4.5f*(u+w)*(u+w)-1.5f*usqr));
	feq[11]=(0.0277777778f*rho*(1.f+3.0f*(v+w)+4.5f*(v+w)*(v+w)-1.5f*usqr));
	feq[12]=(0.0277777778f*rho*(1.f+3.0f*(-u+w)+4.5f*(-u+w)*(-u+w)-1.5f*usqr));
	feq[13]=(0.0277777778f*rho*(1.f+3.0f*(-v+w)+4.5f*(-v+w)*(-v+w)-1.5f*usqr));
	feq[14]=(0.0555555556f*rho*(1.f-3.0f*w+4.5f*w*w-1.5f*usqr));
	feq[15]=(0.0277777778f*rho*(1.f+3.0f*(u-w)+4.5f*(u-w)*(u-w)-1.5f*usqr));
	feq[16]=(0.0277777778f*rho*(1.f+3.0f*(v-w)+4.5f*(v-w)*(v-w)-1.5f*usqr));
	feq[17]=(0.0277777778f*rho*(1.f+3.0f*(-u-w)+4.5f*(-u-w)*(-u-w)-1.5f*usqr));
	feq[18]=(0.0277777778f*rho*(1.f+3.0f*(-v-w)+4.5f*(-v-w)*(-v-w)-1.5f*usqr));
    }
...
inline __device__ void North_Extrap(float* f, float rho)
{
    ...
    float rho1 = f[0]+f[1]+f[2]+f[3]+f[4]+f[5]+f[6]+f[7]+f[8]+f[9]+f[10]+f[11]+f[12]+f[13]+f[14]+f[15]+f[16]+f[17]+f[18];
    if(INCOMP == 0)
    {
    u /= rho1;
    v /= rho1;
                            ...
                            float omega, size_t pitch, int GPU, int zInner, float* FX, float* FY, float* FZ, int t, int flag_F, float* h_interp, size_t pitch_interp, float dpdy)
{
    ...
    if(im == 1 || im ==10){//BB
        ...
        else if(im == 81)//use rho from y+1. all other moments from y-1
        {
            for(int i = 0; i<19; i++)
                   f[i ]= hA[buff_mem(i ,x,y-1,pitch)];
        }
