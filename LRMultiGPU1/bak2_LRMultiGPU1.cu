...
#define LRLEVEL 2
#define LRX0 7.75f	//minimum x coord of LR
#define XLRDIM 32	//number of nodes in x
#define LRY0 7.75f
#define YLRDIM 32
#define LRZ0 -0.25f
#define ZLRDIM 64
...
__global__ void initialize_buffer(float *g, size_t pitch, int yDim)//pitch in elements
{
	int x = threadIdx.x+blockIdx.x*blockDim.x;//coord in linear mem
	int y = threadIdx.y+blockIdx.y*blockDim.y;
	int j = x+y*pitch;//index on padded mem (pitch in elements)
...
int main(int argc, char *argv[])
{
	...
	initialize_buffer<<<g_grid, threads>>>(      g_A_d[n],pitch_elements,YDIM);
	initialize_buffer<<<g_grid, threads>>>(      g_B_d[n],pitch_elements,YDIM);
	initialize_buffer<<<g_grid, threads>>>(      h_A_d[n],pitch_elements,YDIM);
	initialize_buffer<<<g_grid, threads>>>(      h_B_d[n],pitch_elements,YDIM);
	...
	initialize_buffer<<<g_LR_grid, LRthreads>>>(      g_LR_A_d[n],LRpitch_elements,YLRDIM);
	initialize_buffer<<<g_LR_grid, LRthreads>>>(      g_LR_B_d[n],LRpitch_elements,YLRDIM);
	initialize_buffer<<<g_LR_grid, LRthreads>>>(      h_LR_A_d[n],LRpitch_elements,YLRDIM);
	initialize_buffer<<<g_LR_grid, LRthreads>>>(      h_LR_B_d[n],LRpitch_elements,YLRDIM);
	...
	for(int t = 0; t<TMAX; t+=2){
		...
		//update_top   <<<h_grid, threads, 0, stream_halo [n]>>>(      h_A_d[n],      h_B_d[n],f_inner_A_d[n], g_A_d[n],omega,pitch_elements,n,zInner);
		update_bottom<<<h_grid, threads, 0, stream_halo [n]>>>(      g_A_d[n],      g_B_d[n],f_inner_A_d[n],g_temp[n],omega,pitch_elements,n,zInner);
		//update_bottom<<<h_grid, threads, 0, stream_halo [n]>>>(      g_A_d[n],      g_B_d[n],f_inner_A_d[n], h_A_d[n],omega,pitch_elements,n,zInner);
        }
		cudaDeviceSynchronize();
		...
		//update_top_LR_interp   <<<g_LR_grid, LRthreads, 0, stream_halo [n]>>>(h_LR_B_d[n],h_LR_A_d[n],f_inner_LR_B_d[n],h_LR_temp[n],h_B_d[n],g_B_d[n],omegaLR,LRpitch_elements,zLRInner,pitch_elements,zInner,SF_cf,n);
		...
		//update_bottom_LR_interp<<<g_LR_grid, LRthreads, 0, stream_halo [n]>>>(g_LR_B_d[n],g_LR_A_d[n],f_inner_LR_B_d[n],g_LR_temp[n],g_B_d[n],h_B_d[n],omegaLR,LRpitch_elements,zLRInner,pitch_elements,zInner,SF_cf,n);
		
//        update_inner_LR <<<  LRgrid,  LRthreads, 0, stream_inner[n]>>>(f_inner_LR_B_d[n],f_inner_LR_A_d[n],      g_LR_B_d[n], h_LR_B_d[n],omegaLR,LRpitch_elements,n,zLRInner);
//		update_top_LR   <<<g_LR_grid, LRthreads, 0, stream_halo [n]>>>(      h_LR_B_d[n],      h_LR_A_d[n],f_inner_LR_B_d[n],h_LR_temp[n],omegaLR,LRpitch_elements,n,zLRInner);
//		update_bottom_LR<<<g_LR_grid, LRthreads, 0, stream_halo [n]>>>(      g_LR_B_d[n],      g_LR_A_d[n],f_inner_LR_B_d[n],g_LR_temp[n],omegaLR,LRpitch_elements,n,zLRInner);
        }
		...
		//update_top   <<<h_grid, threads, 0, stream_halo [n]>>>(      h_B_d[n],      h_A_d[n],f_inner_B_d[n], g_B_d[n],omega,pitch_elements,n,zInner);
		update_bottom<<<h_grid, threads, 0, stream_halo [n]>>>(      g_B_d[n],      g_A_d[n],f_inner_B_d[n],g_temp[n],omega,pitch_elements,n,zInner);
		//update_bottom<<<h_grid, threads, 0, stream_halo [n]>>>(      g_B_d[n],      g_A_d[n],f_inner_B_d[n], h_B_d[n],omega,pitch_elements,n,zInner);
        }
		cudaDeviceSynchronize();
