...
#define RE 21000.f//2000.f//100.f;
#define UMAX 0.08f
...
//	m8  = 1.666666667f*(-2.f*f9+f10+f11+f12+f13+2.f*f14-f15-f16-f17-f18);
	...
	if(SmagLES == "YES"){
		...
		float Smag = sqrt(PI11*PI11+PI22*PI22+PI33*PI33+2.f*PI12*PI12+2.f*PI23*PI23+2.f*PI13*PI13);
		float Cs = 0.01f;
...
//	cudaFuncSetCacheConfig(LR_d_hybBACD2,cudaFuncCachePreferL1);
	...
	for(int t = 0; t<TMAX; t=t+2){
		if(METHOD == "SINGLE"){
		if(t >= STARTF)
		mrt_d_single_force<<<grid, threads>>>(fA_d,fB_d,omega,pitch_elements,FX_d,FY_d,FZ_d,t);
		else
		...
		if(REFINEMENT == "YES"){
			...
			{
			if(t >= STARTF)
			LR_d_ABCD_force<<<grid2, threads2>>>(fC_d,fD_d,omega2,pitch_elements2,FX_d,FY_d,FZ_d,t);
			else
			...
			LR_d_ABCD<<<grid2, threads2>>>(fC_d,fD_d,omega2,pitch_elements2);
			LR_d_ABDC2<<<grid2, threads2>>>(fD_d,fC_d,omega2,pitch_elements2,SF_cf,2);
			LR_d_ABCD2<<<grid2, threads2>>>(fC_d,fD_d,omega2,pitch_elements2,3);
			LR_d_ABDC_Interp<<<grid2, threads2>>>(fD_d,fC_d,omega2,pitch_elements2,SF_cf);
			}
		...
		if(t >= STARTF)
		mrt_d_single_force<<<grid, threads>>>(fB_d,fA_d,omega,pitch_elements,FX_d,FY_d,FZ_d,t+1);
		else
		...
		if(REFINEMENT == "YES"){
			...
			{
			if(t >= STARTF)
			LR_d_BACD_force<<<grid2, threads2>>>(fC_d,fD_d,omega2,pitch_elements2,FX_d,FY_d,FZ_d,t+1);
			else
			...
			LR_d_BACD<<<grid2, threads2>>>(fC_d,fD_d,omega2,pitch_elements2);
			LR_d_BADC2<<<grid2, threads2>>>(fD_d,fC_d,omega2,pitch_elements2,SF_cf,2);
			LR_d_ABCD2<<<grid2, threads2>>>(fC_d,fD_d,omega2,pitch_elements2,3);
			LR_d_BADC_Interp<<<grid2, threads2>>>(fD_d,fC_d,omega2,pitch_elements2,SF_cf);
			}
...
//			w = fA_h[i+XDIM*YDIM*ZDIM*9]+fA_h[i+XDIM*YDIM*ZDIM*10]+fA_h[i+XDIM*YDIM*ZDIM*11]+fA_h[i+XDIM*YDIM*ZDIM*12]+fA_h[i+XDIM*YDIM*ZDIM*13]-fA_h[i+XDIM*YDIM*ZDIM*14]-fA_h[i+XDIM*YDIM*ZDIM*15]-fA_h[i+XDIM*YDIM*ZDIM*16]-fA_h[i+XDIM*YDIM*ZDIM*17]-fA_h[i+XDIM*YDIM*ZDIM*18];
			...
			output<<col<<", "<<row<<", "<<dep<<", "<<u<<","<<v<<","<<Smag<<","<<rho<<endl;
            if(rho>0.f && rho<2.f){
            }
            else{
                check = 1;
	...
	for(row = 0; row<YLRDIM-0; row++){
		for(col = 0; col<XLRDIM-0; col++){
    		...
    		float PI11 = -1.0f/38.0f*(     (m1)+19.0f* omega2*(m9));
    		float PI22 = -1.0f/76.0f*(2.0f*(m1)-19.0f*(omega2*(m9)-3.0f*omega2*(m11)));
    		float PI33 = -1.0f/76.0f*(2.0f*(m1)-19.0f*(omega2*(m9)+3.0f*omega2*(m11)));
    		float PI12 = -1.5f*omega2*m13;
    		float PI23 = -1.5f*omega2*m14;
    		float PI13 = -1.5f*omega2*m15;
    		float nu0 = ((1.0f/omega2)-0.5f)/3.0f;
    		float Smag = sqrt(PI11*PI11+PI22*PI22+PI33*PI33+2.f*PI12*PI12+2.f*PI23*PI23+2.f*PI13*PI13);
...
//				w = fC_h[i+XLRDIM*YLRDIM*ZLRDIM*9]+fC_h[i+XLRDIM*YLRDIM*ZLRDIM*10]+fC_h[i+XLRDIM*YLRDIM*ZLRDIM*11]+fC_h[i+XLRDIM*YLRDIM*ZLRDIM*12]+fC_h[i+XLRDIM*YLRDIM*ZLRDIM*13]-fC_h[i+XLRDIM*YLRDIM*ZLRDIM*14]-fC_h[i+XLRDIM*YLRDIM*ZLRDIM*15]-fC_h[i+XLRDIM*YLRDIM*ZLRDIM*16]-fC_h[i+XLRDIM*YLRDIM*ZLRDIM*17]-fC_h[i+XLRDIM*YLRDIM*ZLRDIM*18];
			output<<LRX0+col*LRFACTOR<<", "<<LRY0+row*LRFACTOR<<", "<<LRZ0+dep*LRFACTOR<<", "<<u<<","<<v<<","<<Smag<<","<<rho<<endl;
