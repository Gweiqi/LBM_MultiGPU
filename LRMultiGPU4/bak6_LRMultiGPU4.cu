...
#define LRLEVEL 4
#define LRX0 31.125f    //minimum x coord of LR
...
__global__ void InterpCF(float* f_f, float* g_f, float* h_f, size_t pitch_f, float* m_f_c, float* m_g_c, float* m_h_c, float* m_g_temp, float* m_h_temp, size_t pitch_m, float SF, float omega_c, int zInner, int zInner_f)
{
    ...
    int y = threadIdx.y+blockIdx.y*blockDim.y;
    int z = threadIdx.z+blockIdx.z*blockDim.z;
    __shared__ float mom_c[9][BLOCKSIZEINTERP][2][2];
    __shared__ float S_c[6][BLOCKSIZEINTERP][2][2];
...
__global__ void InterpFC(float* f_c, float* g_c, float* h_c, float* f_f, float* h_f, float* temp_f, size_t pitch_c, size_t pitch_f, float SF, float omega_f, int zInner, int zInner_f)
{
    ...
    {
        float f[19];
        float mom[8][6];//physical moments of 8 neighboring nodes
        float S_f[8][6];//strain rate tensor of 8 neighboring nodes
//        int xLR[8],yLR[8],zLR[8];
//        xLR[0] = LRLEVEL*(x-LRX0);
//        xLR[1] = XLR[0]+1; xLR[3] = XLR[0]+1; xLR[5] = XLR[0]+1; xLR[7] = XLR[0]+1; 
//        xLR[2] = XLR[0];   xLR[4] = XLR[0];   xLR[6] = XLR[0]; 
//        yLR[0] = LRLEVEL*(y-LRY0);
//        yLR[1] = YLR[0]+1; yLR[3] = YLR[0]+1; yLR[5] = YLR[0]+1; yLR[7] = YLR[0]+1; 
//        yLR[2] = YLR[0];   yLR[4] = YLR[0];   yLR[6] = YLR[0]; 
//        zLR[0] = LRLEVEL*(z-(1.f-0.5f*LRFACTOR));
//        zLR[1] = ZLR[0]+1; zLR[3] = ZLR[0]+1; zLR[5] = ZLR[0]+1; zLR[7] = ZLR[0]+1; 
//        zLR[2] = ZLR[0];   zLR[4] = ZLR[0];   zLR[6] = ZLR[0]; 
        //top nodes. interp between h and h_temp. output to h
        if(z == zInner+1)
        {
            for(int i = 0; i<19; i++)
                f[i] = h_f[buff_memLR(i,0,0,pitch_f)];
...
//            for(int n = 0; n<4; n++){
//            for(int i = 0; i<19; i++)
//                f[i] = h_f[buff_memLR(i,xLR[n],yLR[n],pitch_f)];
//            PhysicalMoments(mom[n],f);
//            }
//            for(int n = 4; n<8; n++){
//            for(int i = 0; i<19; i++)
//                f[i] = f_f[buff_memLR(i,xLR[n],yLR[n],zInner-1,pitch_f)];
//            PhysicalMoments(mom[n],f);
//            }
//        }
...
int main(int argc, char *argv[])
{
    ...
    for(int t = 0; t<TMAX; t++)
    {
        ...
        for(int n = 0; n<GPU_N; n++){
        cudaSetDevice(n);
        InterpCF<<<Interp_grid,Interp_threads,0,stream_inner[n]>>>(f_LR_d[n][C],g_LR_d[n][C],h_LR_d[n][C],pitchLR_e,f_interp[n],g_interp[n],h_interp[n],g_interp_temp[n],h_interp_temp[n],pitchInterp_e,SF_cf,omega,zInner,zLRInner);
        //cudaDeviceSynchronize();
        }
        ...
        for(int n = 0; n<GPU_N; n++){
        cudaSetDevice(n);
        InterpFC<<<Interp_grid,Interp_threads,0,stream_halo[n]>>>(f_d[n][A],g_d[n][A],h_d[n][A],f_LR_d[n][C],h_LR_d[n][C],h_LR_temp[n],pitch_e,pitchLR_e,SF_fc,omegaLR,zInner,zLRInner);
        }
