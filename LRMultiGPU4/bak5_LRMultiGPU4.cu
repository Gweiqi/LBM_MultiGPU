...
#define LRLEVEL 4
#define LRX0 15.125f    //minimum x coord of LR
...
*/
__global__ void InterpCF(float* f_f, float* g_f, float* h_f, size_t pitch_f, float* m_f_c, float* m_g_c, float* m_h_c, float* m_g_temp, float* m_h_temp, size_t pitch_m, float SF, float omega_c, int zInner, int zInner_f)
{
    int x = threadIdx.x+blockIdx.x*blockDim.x;
    int y = threadIdx.y+blockIdx.y*blockDim.y;
    int z = threadIdx.z+blockIdx.z*blockDim.z;
    __shared__ float mom_c[9][BLOCKSIZELRX][2][2];
    __shared__ float S_c[6][BLOCKSIZELRX][2][2];
    if(blockIdx.z == 0 && threadIdx.x<BLOCKSIZELRX*LRFACTOR+1 && threadIdx.z<2 && threadIdx.y<2)
    {
        //use g and g_temp
        ...
        StrainRate(S,m_strain,omega_c);
        for(int i = 0; i<6; i++)
        S_c[i][threadIdx.x][threadIdx.y][threadIdx.z]= S[i];
    }
    else if(blockIdx.z == 1 && threadIdx.x<BLOCKSIZELRX*LRFACTOR+1 && threadIdx.z<2 && threadIdx.y<2)
    {
        //use g and f   
        int x_c = threadIdx.x+blockIdx.x*BLOCKSIZELRX*LRFACTOR;//in coarse grid, blockdim.x is LRX*LRFACTOR
        int y_c = threadIdx.y+blockIdx.y;//in coarse grid, blockdim.y is 1
    ...
    else if(blockIdx.z == zInner+1 && threadIdx.x<BLOCKSIZELRX*LRFACTOR+1 && threadIdx.z<2 && threadIdx.y<2)
    {
        //use h and f   
        int x_c = threadIdx.x+blockIdx.x*BLOCKSIZELRX*LRFACTOR;//in coarse grid, blockdim.x is LRX*LRFACTOR
        int y_c = threadIdx.y+blockIdx.y;//in coarse grid, blockdim.y is 1
    ...
    else if(threadIdx.x<BLOCKSIZELRX*LRFACTOR+1 && threadIdx.z<2 && threadIdx.y<2){//use f only
        int x_c = threadIdx.x+blockIdx.x*BLOCKSIZELRX*LRFACTOR;//in coarse grid, blockdim.x is LRX*LRFACTOR
        int y_c = threadIdx.y+blockIdx.y;//in coarse grid, blockdim.y is 1
    ...
    u_x1=S_c[0][xm][0][0];v_y1=S_c[1][xm][0][0];w_z1=S_c[2][xm][0][0];Sxy1=S_c[3][xm][0][0];Syz1=S_c[4][xm][0][0];Sxz1=S_c[5][xm][0][0];
    u_x2=S_c[0][xp][0][0];v_y2=S_c[1][xp][0][0];w_z2=S_c[2][xp][0][0];Sxy2=S_c[3][xp][0][0];Syz2=S_c[4][xp][0][0];Sxz2=S_c[5][xp][0][0];
    u_x3=S_c[0][xm][1][0];v_y3=S_c[1][xm][1][0];w_z3=S_c[2][xm][1][0];Sxy3=S_c[3][xm][1][0];Syz3=S_c[4][xm][1][0];Sxz3=S_c[5][xm][1][0];
    u_x4=S_c[0][xp][1][0];v_y4=S_c[1][xp][1][0];w_z4=S_c[2][xp][1][0];Sxy4=S_c[3][xp][1][0];Syz4=S_c[4][xp][1][0];Sxz4=S_c[5][xp][1][0];
    u_x5=S_c[0][xm][0][1];v_y5=S_c[1][xm][0][1];w_z5=S_c[2][xm][0][1];Sxy5=S_c[3][xm][0][1];Syz5=S_c[4][xm][0][1];Sxz5=S_c[5][xm][0][1];
    u_x6=S_c[0][xp][0][1];v_y6=S_c[1][xp][0][1];w_z6=S_c[2][xp][0][1];Sxy6=S_c[3][xp][0][1];Syz6=S_c[4][xp][0][1];Sxz6=S_c[5][xp][0][1];
    u_x7=S_c[0][xm][1][1];v_y7=S_c[1][xm][1][1];w_z7=S_c[2][xm][1][1];Sxy7=S_c[3][xm][1][1];Syz7=S_c[4][xm][1][1];Sxz7=S_c[5][xm][1][1];
    u_x8=S_c[0][xp][1][1];v_y8=S_c[1][xp][1][1];w_z8=S_c[2][xp][1][1];Sxy8=S_c[3][xp][1][1];Syz8=S_c[4][xp][1][1];Sxz8=S_c[5][xp][1][1];

    float m03,m05,m07, m13,m15,m17, m23,m25,m27, m33,m35,m37, m43,m45,m47, m53,m55,m57, m63,m65,m67, m73,m75,m77;
    m03=mom_c[1][xm][0][0];m05=mom_c[2][xm][0][0];m07=mom_c[3][xm][0][0];
    m13=mom_c[1][xp][0][0];m15=mom_c[2][xp][0][0];m17=mom_c[3][xp][0][0];
    m23=mom_c[1][xm][1][0];m25=mom_c[2][xm][1][0];m27=mom_c[3][xm][1][0];
    m33=mom_c[1][xp][1][0];m35=mom_c[2][xp][1][0];m37=mom_c[3][xp][1][0];
    m43=mom_c[1][xm][0][1];m45=mom_c[2][xm][0][1];m47=mom_c[3][xm][0][1];
    m53=mom_c[1][xp][0][1];m55=mom_c[2][xp][0][1];m57=mom_c[3][xp][0][1];
    m63=mom_c[1][xm][1][1];m65=mom_c[2][xm][1][1];m67=mom_c[3][xm][1][1];
    m73=mom_c[1][xp][1][1];m75=mom_c[2][xp][1][1];m77=mom_c[3][xp][1][1];
...
int main(int argc, char *argv[])
{
    ...
    dim3 Interp_grid(((XLRDIM+BLOCKSIZELRX-1)/BLOCKSIZELRX),((YLRDIM+LRLEVEL-1)/LRLEVEL),ZLRDIM/LRLEVEL/GPU_N);
    cout<<((XLRDIM+BLOCKSIZELRX-1)/BLOCKSIZELRX)<<", "<<((YLRDIM+LRLEVEL-1)/LRLEVEL)<<", "<<ZLRDIM/LRLEVEL/GPU_N<<endl;
    ...
    for(int t = 0; t<TMAX; t++)
    {
        ...
        for(int n = 0; n<GPU_N; n++){
        cudaSetDevice(n);
        InterpCF<<<Interp_grid,Interp_threads,0,0>>>(f_LR_d[n][C],g_LR_d[n][C],h_LR_d[n][C],pitchLR_e,f_interp[n],g_interp[n],h_interp[n],g_interp_temp[n],h_interp_temp[n],pitchInterp_e,SF_cf,omega,zInner,zLRInner);
        cudaDeviceSynchronize();
        }
...
//    output<<"VARIABLES = \"X\",\"Y\",\"Z\",\"u\"\n";
//    output<<"ZONE F=POINT, I="<<XLRDIM*LRFACTOR+1<<", J="<<YLRDIM*LRFACTOR+1<<", K="<<zInner<<"\n";
//        for(int k = 0; k< zInner; k++){
//        for(int j = 0; j< YLRDIM*LRFACTOR+1; j++){
//        for(int i = 0; i< XLRDIM*LRFACTOR+1; i++){
//            output<<i<<", "<<j<<", "<<k<<", "<<interp_h[n][int(i+j*(XLRDIM*LRFACTOR+1)+k*(YLRDIM*LRFACTOR+1)*(XLRDIM*LRFACTOR+1))]<<endl;
//        }
//        }
//        }
