...
#define LRLEVEL 2
#define LRX0 15.25f    //minimum x coord of LR
...
inline __device__ void ScaleMoments_cf(float* m, float SF)
{
m[ 1] = (-11.f*m[0]+19.f*(m[3]*m[3]+m[5]*m[5]+m[7]*m[7]));
m[ 2] =  7.53968254f*(m[3]*m[3]+m[5]*m[5]+m[7]*m[7]);
m[ 4] = -0.666666667f*m[3];
m[ 6] = -0.666666667f*m[5];
m[ 8] = -0.666666667f*m[7];
m[ 9] = m[9]*SF+(1.f-SF)*(2.f*m[3]*m[3]-(m[5]*m[5]+m[7]*m[7]));
m[10] = 0.f;//m[10]*SF;
m[11] = m[11]*SF+(1.f-SF)*(m[5]*m[5]-m[7]*m[7]);
m[12] = 0.f;//m[12]*SF;
m[13] = m[13]*SF+(1.f-SF)*m[3]*m[5];
m[14] = m[14]*SF+(1.f-SF)*m[5]*m[7];
m[15] = m[15]*SF+(1.f-SF)*m[3]*m[7];
m[16] = 0.f;//m[16]*SF;
m[17] = 0.f;//m[17]*SF;
m[18] = 0.f;//m[18]*SF;
...
*/
__global__ void InterpCF(float* f_f, float* g_f, float* h_f, size_t pitch_f, float* m_f_c, float* m_g_c, float* m_h_c, float* m_g_temp, float* m_h_temp, size_t pitch_m, float SF, int zInner, int zInner_f)
{
    int x = threadIdx.x+blockIdx.x*blockDim.x;
    int y = threadIdx.y+blockIdx.y*blockDim.y;
    int z = threadIdx.z+blockIdx.z*blockDim.z;
    __shared__ float mom_c[19][BLOCKSIZELRX][2][2];//should be BLOCKSIZELRX*LRFACTOR+1
                  ...
                  float **velFluc, float omega, int GPU_N, int GPU)
{
    ...
    for(int i = 0; i<XDIM; i++){
        ...
        float m1neq = m[ 1]-(-11.f*m[0]+19.f*(m[3]*m[3]+m[5]*m[5]+m[7]*m[7]));
        float m2neq = m[ 2]-7.53968254f*(m[3]*m[3]+m[5]*m[5]+m[7]*m[7]);
        float m4neq = m[ 4]+0.666666667f*m[3];
        float m9neq = m[ 9]-(2.f*m[3]*m[3]-(m[5]*m[5]+m[7]*m[7]));
...
int main(int argc, char *argv[])
{
    ...
    for(int t = 0; t<TMAX; t++)
    {
        ...
        {
        for(int n = 0; n<GPU_N; n++)
            cudaMemcpyPeerAsync(&h_interp_temp[n][0],n,&g_interp[   (n+1)%GPU_N][0],   (n+1)%GPU_N,pitchInterp_e*(YLRDIM*LRFACTOR+1)*sizeof(float)*19,stream_halo[n]);
        for(int n = 0; n<GPU_N; n++)
        ...
        for(int n = 0; n<GPU_N; n++){
        cudaSetDevice(n);
        InterpCF<<<Interp_grid,Interp_threads,0,0>>>(f_LR_d[n][C],g_LR_d[n][C],h_LR_d[n][C],pitchLR_e,f_interp[n],g_interp[n],h_interp[n],g_interp_temp[n],h_interp_temp[n],pitchInterp_e,SF_cf,zInner,zLRInner);
        cudaDeviceSynchronize();
        }
