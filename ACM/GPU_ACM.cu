...
__global__ void ACM_U_Shared(float* uA, float* vA, float* Res, float* uB, float* vB, float* pB, float* uC, float* vC, float* uD, float* vD, float nu, int it, int t, size_t pitch)
{
    ...
    float De,Dw,Dn,Ds;
    float B_Ue,B_Vn;
    Fe = 0.5f*(u[tx  ][ty  ]+u[tx+1][ty  ]);
    Fw = 0.5f*(u[tx-1][ty  ]+u[tx  ][ty  ]);
    Fn = 0.5f*(v[tx  ][ty  ]+v[tx+1][ty  ]);
    Fs = 0.5f*(v[tx  ][ty-1]+v[tx+1][ty-1]);
    De = nu; Dw = nu; Dn = nu; Ds = nu;
    if(CONV == 1){
    AE_Ue = max(-Fe,0.f)+De;
    AW_Ue = max( Fw,0.f)+Dw;
    ...
    uA[ x  + y   *pitch] =( AE_Ue*u[tx+1][ty  ]+AW_Ue*u[tx-1][ty  ]
                           ...
                           -AP_Ue*u[tx  ][ty  ]+ B_Ue
                           -( 3.f*u[tx  ][ty  ]-4.f*uC[x  +(y  )*pitch]+uD[x  +(y  )*pitch])*0.5f/DT)*DTAU
                           + u[tx  ][ty  ];
    Fe = 0.5f*(u[tx  ][ty  ]+u[tx  ][ty+1]);
    Fw = 0.5f*(u[tx-1][ty  ]+u[tx-1][ty+1]);
    Fn = 0.5f*(v[tx  ][ty  ]+v[tx  ][ty+1]);
    Fs = 0.5f*(v[tx  ][ty  ]+v[tx  ][ty-1]);
    De = nu; Dw = nu; Dn = nu; Ds = nu;
    if(CONV == 1){
    AE_Vn = max(-Fe,0.f)+De;
    AW_Vn = max( Fw,0.f)+Dw;
    ...
    vA[ x  + y   *pitch] =( AE_Vn*v[tx+1][ty  ]+AW_Vn*v[tx-1][ty  ]
                           ...
                           -AP_Vn*v[tx  ][ty  ]+ B_Vn
                           -( 3.f*v[tx  ][ty  ]-4.f*vC[x  +(y  )*pitch]+vD[x  +(y  )*pitch])*0.5f/DT)*DTAU
                           + v[tx  ][ty  ];
