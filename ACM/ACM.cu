...
__global__ void ACM_P(float* pA, float* Res, float* uA, float* vA, float* pB, float *pC, float dt, int it, int t, size_t pitch, float *uD)
{
    int x = threadIdx.x+blockIdx.x*blockDim.x;//coord in linear mem
    int y = threadIdx.y+blockIdx.y*blockDim.y;
...
int main()
{
    ...
    for(int it = 0; it<maxit; it++){
        ...
        ACM_P<<<grid,threads,0,compute>>>(p_d[A],Res_d,u_d[A],v_d[A],p_d[B],p_d[C],dt,it,t,pitch_e,u_d[D]);
        cudaDeviceSynchronize();
