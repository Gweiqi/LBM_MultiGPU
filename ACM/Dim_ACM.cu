...
#define RE 100
#define UMAX 20.f
#define BETA 0.0001f //0.0001f //0.01f //beta = 1/c^2
#define DTAU 0.004f //0.001f
#define DT 0.1f
#define DT1 0.1f
#define MAXIT1 100
#define STARTRAMP 1000
#define ENDRAMP 2000

#define CONV 2  //1:UDS

float timestep(int t)
{
    float dt = DT;
    if(t<STARTRAMP) dt = DT1;
    else if(t>=STARTRAMP && t<ENDRAMP)
        dt = DT1-(DT1-DT)/(ENDRAMP-STARTRAMP)*(t-STARTRAMP);
    if(t == STARTRAMP) cout<<"Ramping down time step from: "<<DT1<<endl;
    else if(t == ENDRAMP) cout<<"Finished ramping down time step to: "<<dt<<endl;
    return dt;
}

float iterations(int t)
{
    int it = MAXIT;
    if(t<STARTRAMP) it = MAXIT1;
    else if(t>=STARTRAMP && t<ENDRAMP)
        it = MAXIT1-(MAXIT1-MAXIT)/(ENDRAMP-STARTRAMP)*(t-STARTRAMP);
    if(t == STARTRAMP) cout<<"Ramping up max iterations from: "<<it<<endl;
    else if(t == ENDRAMP) cout<<"Finished ramping up max iterations to: "<<it<<endl;
    return it;
}
...
__global__ void ACM_U_Shared(float* uA, float* vA, float* Res, float* uB, float* vB, float* pB, float* uC, float* vC, float* uD, float* vD, float nu, float dt, int it, int t, size_t pitch)
{
    ...
    uA[ x  + y   *pitch] =(((AE_Ue*u[tx+1][ty  ]+AW_Ue*u[tx-1][ty  ]
                           ...
                           //-(-4.f*uC[x  +(y  )*pitch]+uD[x  +(y  )*pitch])*0.5f/dt
                           -( 3.f*u[tx  ][ty  ]-4.f*uC[x  +(y  )*pitch]+uD[x  +(y  )*pitch])*0.5f/dt
                           //-( u[tx  ][ty  ]-uC[x  +(y  )*pitch])/dt
                           )*DTAU)///(1.f+3.f*DTAU/dt)
                           + u[tx  ][ty  ];
//    uA[ x  + y   *pitch] =(((AE_Ue*u[tx+1][ty  ]+AW_Ue*u[tx-1][ty  ]
//                           +AN_Ue*u[tx  ][ty+1]+AS_Ue*u[tx  ][ty-1]
//                           -AP_Ue*u[tx  ][ty  ]+ B_Ue)/(dx*dy)
...
//                           ))/(1.f/DT+1.f/DTAU);
    ...
    vA[ x  + y   *pitch] =(((AE_Vn*v[tx+1][ty  ]+AW_Vn*v[tx-1][ty  ]
                           ...
                           //-(-4.f*vC[x  +(y  )*pitch]+vD[x  +(y  )*pitch])*0.5f/dt
                           -( 3.f*v[tx  ][ty  ]-4.f*vC[x  +(y  )*pitch]+vD[x  +(y  )*pitch])*0.5f/dt
                           //-( v[tx  ][ty  ]-vC[x  +(y  )*pitch])/dt
                           )*DTAU)///(1.f+3.f*DTAU/dt)
                           + v[tx  ][ty  ];
//    vA[ x  + y   *pitch] =(((AE_Vn*v[tx+1][ty  ]+AW_Vn*v[tx-1][ty  ]
//                           +AN_Vn*v[tx  ][ty+1]+AS_Vn*v[tx  ][ty-1]
...
__global__ void ACM_P(float* pA, float* Res, float* uA, float* vA, float* pB, float *pC, float dt, int it, int t, size_t pitch, float *uD, float *test)
{
    int x = threadIdx.x+blockIdx.x*blockDim.x;//coord in linear mem
    int y = threadIdx.y+blockIdx.y*blockDim.y;
    ...
    pA[ x  + y   *pitch] = -(res/BETA)*DTAU+pB[ x  + y   *pitch];
    if(x > HALO+2 && x < XDIM-HALO-2 && y > HALO+2 && y < YDIM-HALO-2 
...
int main()
{
    ...
    for(int it = 0; it<maxit; it++){
        swap(A,B);
        //if(it > 0)
        ACM_U_Shared<<<u_grid,u_threads,0,compute>>>(u_d[A],v_d[A],Res_d,u_d[B],v_d[B],p_d[B],u_d[C],v_d[C],u_d[D],v_d[D],nu,dt,it,t,pitch_e);
        ACM_BC<<<grid,threads,0,compute>>>(u_d[A],v_d[A],t,pitch_e);
        ACM_P<<<grid,threads,0,compute>>>(p_d[A],Res_d,u_d[A],v_d[A],p_d[B],p_d[C],dt,it,t,pitch_e,u_d[D],test);
        cudaDeviceSynchronize();
