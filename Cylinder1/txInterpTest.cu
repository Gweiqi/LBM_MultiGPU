...
#define LRX0 415.75	//minimum x coord of LR
#define XLRDIM 384	//number of nodes in x
#define LRY0 319.75
#define YLRDIM 256
...
inline __device__ void DirichletWest(float& f0, float& f1, float& f2,
						   float& f3, float& f4, float& f5,
						   float& f6, float& f7, float& f8, int y)
{
	if(y == 0){
...
//	f1 = f3 -0.1111111111f*(rho-3.0f*u+4.5f*u*u-1.5f*usqr);
//	f5 = f7 -0.02777777778*(rho+3.0f*(-u-v)+4.5f*(-u-v)*(-u-v)-1.5f*usqr);
//	f6 = f6 -0.02777777778*(rho+3.0f*(-u+v)+4.5f*(-u+v)*(-u+v)-1.5f*usqr);
}
							...
							float omega, size_t pitch, float SF)//pitch in elements
{
	...
	if(x < LRLEVEL || x > XLRDIM-1-LRLEVEL || y < LRLEVEL || y > YLRDIM-1-LRLEVEL)
	//if(x < 2 || x > XLRDIM-4 || y < 2 || y > YLRDIM-4)
	//if(false)
	{
...
//	}
	...
	texRef_f0C.addressMode[i]  = cudaAddressModeClamp;
	texRef_f1C.addressMode[i]  = cudaAddressModeClamp;
	texRef_f2C.addressMode[i]  = cudaAddressModeClamp;
	texRef_f3C.addressMode[i]  = cudaAddressModeClamp;
	texRef_f4C.addressMode[i]  = cudaAddressModeClamp;
	texRef_f5C.addressMode[i]  = cudaAddressModeClamp;
	texRef_f6C.addressMode[i]  = cudaAddressModeClamp;
	texRef_f7C.addressMode[i]  = cudaAddressModeClamp;
	texRef_f8C.addressMode[i]  = cudaAddressModeClamp;

	texRef_f0D.addressMode[i]  = cudaAddressModeClamp;
	texRef_f1D.addressMode[i]  = cudaAddressModeClamp;
	texRef_f2D.addressMode[i]  = cudaAddressModeClamp;
	texRef_f3D.addressMode[i]  = cudaAddressModeClamp;
	texRef_f4D.addressMode[i]  = cudaAddressModeClamp;
	texRef_f5D.addressMode[i]  = cudaAddressModeClamp;
	texRef_f6D.addressMode[i]  = cudaAddressModeClamp;
	texRef_f7D.addressMode[i]  = cudaAddressModeClamp;
	texRef_f8D.addressMode[i]  = cudaAddressModeClamp;
	}
	}
	...
	for(int t = 0; t<TMAX; t=t+2){
		if(REFINEMENT == "YES"){
			...
			LR_d_BADC<<<grid2, threads2>>>(fD_d,fC_d,omega2,pitch_elements2,SF_cf);
				...
				//LR_d_hybBACD2<<<grid2, threads2>>>(fC_d,fD_d,omega2,pitch_elements2,1);
...
//			<<double((XDIM*YDIM*ZDIM)*double(TMAX/1000000.f))/restime<<"MLUPS)"<<endl;
	...
	if(REFINEMENT == "YES"){
	cudaMemcpy2D(fC_h,XLRDIM*sizeof(float),fD_d,pitch2,XLRDIM*sizeof(float),YLRDIM*9,cudaMemcpyDeviceToHost);
	}	
	...
	for(row = 0; row<YDIM; row++){
		for(col = 0; col<XDIM; col++){
			...
			//output<<col<<", "<<row<<", "<<fA_h[i+XDIM*YDIM*1]<<","<<fA_h[i+XDIM*YDIM*5]<<","<<fA_h[i+XDIM*YDIM*8]<<endl;
		}
	}
