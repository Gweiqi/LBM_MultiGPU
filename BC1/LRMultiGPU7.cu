...
#define LRLEVEL 2
#define LRX0 135.25f    //minimum x coord of LR
#define XLRDIM 96    //number of nodes in x
#define LRY0 129.25f
#define YLRDIM 160
#define LRZ0 -0.75f
#define ZLRDIM 16
...
//#define LRLEVEL 4
//#define LRX0 135.125f    //minimum x coord of LR
//#define XLRDIM 192    //number of nodes in x
//#define LRY0 129.125f
//#define YLRDIM 320
//#define LRZ0 -0.875f
//#define ZLRDIM 320
//#define ORDER 2 //order of accuracy of interpolation

#define RE 10.f //3000.f//2000.f//100.f;
#define UMAX 0.08f
...
#define VELAV 1
#define START_VELAV   80000
#define START_VELFLUC 140000
...
inline __device__ void mrt_collide(float* f, float omega, float dt)
{
    float m[19];
    //float u,v,w,rho;    
    //float usqr;
...
float Q = sqrt(PI11*PI11+PI22*PI22+PI33*PI33+2.f*PI12*PI12+2.f*PI23*PI23+2.f*PI13*PI13);
    ...
    dt = 1.f;
    float tau = 0.5f*tau0+0.5f*sqrt(tau0*tau0+18.f*CS*sqrt(2.f)*Q*dt*dt);//added *dt*dt (4/4/2013)
    omega = 1.f/tau;
}
...
//mom[3]=0.f;
//
//InvertPhysicalMoments(f,mom,1.f);
...
inline __device__ void West_Extrap(float* f, float u, float rho)
{
    float m[19];
...
inline __device__ void West_Reg(float* f, float u, int t)
{
    float m[19];
    float v = 0.f;//f[ 1]-f[ 3]+f[ 5]-f[ 6]-f[ 7]+f[ 8]+f[10]-f[12]+f[15]-f[17];
    float w = 0.f;//f[ 9]+f[10]+f[11]+f[12]+f[13]-f[14]-f[15]-f[16]-f[17]-f[18];
                            ...
                            float omega, size_t pitch, int GPU, int zInner, float* FX, float* FY, float* FZ, int t, int flag_F, float* h_interp, size_t pitch_interp)
{
    ...
    if(im == 1 || im ==10){//BB
        ...
        {
            float rho = 2.f*(f[3]+f[6]+f[7]+f[12]+f[17])+(f[0]+f[2]+f[4]+f[9]+f[11]+f[13]+f[14]+f[16]+f[18]);
            rho /= (1.f+UMAX);

            for(int i = 0; i<19; i++)
                   f[i ]= hA[buff_mem(i ,x+1,y,pitch)];
            float u_in = PoisProf3D(y,(GPU+1)*(zInner+2)-1);
            West_Extrap(f,u_in,rho);
        }
        if(im == 25)
...
//        float PI13 = -1.5f*omega*m[15];
        ...
        float PI11 = -0.5f*m[ 9];
        float PI22 = 0.25f*(m[ 9]-3.0f*m[11]);
        float PI33 = 0.25f*(m[ 9]+3.0f*m[11]);
...
PI11 = (f[1 ]-feq1 )+(f[3 ]-feq3 )+(f[5 ]-feq5 )+(f[6 ]-feq6 )+(f[7 ]-feq7 )+(f[8 ]-feq8 )+(f[10]-feq10)+(f[12]-feq12)+(f[15]-feq15)+(f[17]-feq17);
PI22 = (f[2 ]-feq2 )+(f[4 ]-feq4 )+(f[5 ]-feq5 )+(f[6 ]-feq6 )+(f[7 ]-feq7 )+(f[8 ]-feq8 )+(f[11]-feq11)+(f[13]-feq13)+(f[16]-feq16)+(f[18]-feq18);
PI33 = (f[9 ]-feq9 )+(f[14]-feq14)+(f[10]-feq10)+(f[12]-feq12)+(f[15]-feq15)+(f[17]-feq17)+(f[11]-feq11)+(f[13]-feq13)+(f[16]-feq16)+(f[18]-feq18);
PI12 = (f[5 ]-feq5 )+(f[7 ]-feq7 )-(f[6 ]-feq6 )-(f[8 ]-feq8 );
PI13 = (f[10]-feq10)+(f[17]-feq17)-(f[12]-feq12)-(f[15]-feq15);
PI23 = (f[11]-feq11)+(f[18]-feq18)-(f[13]-feq13)-(f[16]-feq16);
