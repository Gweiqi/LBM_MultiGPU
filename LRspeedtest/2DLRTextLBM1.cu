...
//__global__ void TextInterp_CF(float* fin, float* fout,
...
//}
//
//__global__ void SMInterp_CF(float* fin, float* fout,
//							float omega, size_t pitch, float SF)//pitch in elements
//{
//	int x = threadIdx.x+blockIdx.x*blockDim.x;//coord in linear mem
//	int y = threadIdx.y+blockIdx.y*blockDim.y;
...
//	int j = x+y*pitch;//index on padded mem (pitch in elements)
//	float f0,f1,f2,f3,f4,f5,f6,f7,f8;
//	__shared__ float f0_s[BLOCKDIMX+4];
//	__shared__ float f1_s[BLOCKDIMX+4];
//	__shared__ float f2_s[BLOCKDIMX+4];
//	__shared__ float f3_s[BLOCKDIMX+4];
//	__shared__ float f4_s[BLOCKDIMX+4];
//	__shared__ float f5_s[BLOCKDIMX+4];
//	__shared__ float f6_s[BLOCKDIMX+4];
//	__shared__ float f7_s[BLOCKDIMX+4];
//	__shared__ float f8_s[BLOCKDIMX+4];
//
//	if(x < 2 || x > XLRDIM-3 || y < 2 || y > YLRDIM-3)
//	{
//	int x1 = int(xcoord);
//	int y1 = int(ycoord);
//	f0_s[threadIdx.x] = f0[f_memLR(0,x1,y1,pitch)];
//	f1_s[threadIdx.x] = f1[f_memLR(1,x1,y1,pitch)];
//	f2_s[threadIdx.x] = f2[f_memLR(2,x1,y1,pitch)];
//	f3_s[threadIdx.x] = f3[f_memLR(3,x1,y1,pitch)];
//	f4_s[threadIdx.x] = f4[f_memLR(4,x1,y1,pitch)];
//	f5_s[threadIdx.x] = f5[f_memLR(5,x1,y1,pitch)];
//	f6_s[threadIdx.x] = f6[f_memLR(6,x1,y1,pitch)];
//	f7_s[threadIdx.x] = f7[f_memLR(7,x1,y1,pitch)];
//	f8_s[threadIdx.x] = f8[f_memLR(8,x1,y1,pitch)];
//
//	if(threadIdx.x == BLOCKDIMX-1){
//	f0_s[threadIdx.x*2] = f0[f_memLR(0,x1,y1,pitch)];
//	f1_s[threadIdx.x*2] = f1[f_memLR(1,x1,y1,pitch)];
//	f2_s[threadIdx.x*2] = f2[f_memLR(2,x1,y1,pitch)];
//	f3_s[threadIdx.x*2] = f3[f_memLR(3,x1,y1,pitch)];
//	f4_s[threadIdx.x*2] = f4[f_memLR(4,x1,y1,pitch)];
//	f5_s[threadIdx.x*2] = f5[f_memLR(5,x1,y1,pitch)];
//	f6_s[threadIdx.x*2] = f6[f_memLR(6,x1,y1,pitch)];
//	f7_s[threadIdx.x*2] = f7[f_memLR(7,x1,y1,pitch)];
//	f8_s[threadIdx.x*2] = f8[f_memLR(8,x1,y1,pitch)];
//	}
//
//
//	y1++;
...
//	float ycoord = y+0.5f;
	...
	f2 = fin[f_mem(2 ,x  ,y-1,pitch)];
	f4 = fin[f_mem(4 ,x  ,y+1,pitch)];
//	f1 = tex2D(texRef_f1B ,xcoord-1,ycoord  );
//	f3 = tex2D(texRef_f3B ,xcoord+1,ycoord  );
//	f5 = tex2D(texRef_f5B ,xcoord-1,ycoord-1);
//	f6 = tex2D(texRef_f6B ,xcoord+1,ycoord-1);
//	f7 = tex2D(texRef_f7B ,xcoord+1,ycoord+1);
//	f8 = tex2D(texRef_f8B ,xcoord-1,ycoord+1);
...
//			<<double((XDIM*YDIM*ZDIM)*double(TMAX/1000000.f))/restime<<"MLUPS)"<<endl;
	...
	cudaMemcpy2D(fA_h,XDIM*sizeof(float),fB_d,pitch,XDIM*sizeof(float),YDIM*9,cudaMemcpyDeviceToHost);
	if(REFINEMENT == "YES"){
