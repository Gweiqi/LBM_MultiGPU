...
#define PRERUN 1950 //p-control constant

#define OBSTR1 10.f
#define OBSTX1 40.5f
#define OBSTY1 110.5f
#define OBSTZ1 32.5f
...
#define RE 100.f //1080.f//2000.f//100.f;
#define UMAX 0.04f
                            ...
                            float omega, size_t pitch, int GPU, int zInner, float* FX, float* FY, float* FZ, int t, int flag_F, float* h_interp, size_t pitch_interp, float dpdy)
{
    ...
    if(im == 1 || im ==10){//BB
        ...
        else if(im == 60)//south inlet periodic
        {
            ...
            float ft[19];
            for(int i = 0; i<19; i++)
                   ft[i] = hA[buff_mem(i ,x,y-1,pitch)];
    float u = ft[ 1]-ft[ 3]+ft[ 5]-ft[ 6]-ft[ 7]+ft[ 8]+ft[10]-ft[12]+ft[15]-ft[17];
    float v = ft[ 2]-ft[ 4]+ft[ 5]+ft[ 6]-ft[ 7]-ft[ 8]+ft[11]-ft[13]+ft[16]-ft[18];
    float w = ft[ 9]+ft[10]+ft[11]+ft[12]+ft[13]-ft[14]-ft[15]-ft[16]-ft[17]-ft[18];
            for(int i = 0; i<19; i++)
                   f[i ]= hA[buff_mem(i ,x,y+1,pitch)];
            South_Extrap(f,u);
            //if(y>DYNY1) dpdy = 0.f;
