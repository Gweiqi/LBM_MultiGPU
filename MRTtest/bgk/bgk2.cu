...
#define LRLEVEL 2
#define LRX0 73.25f    //minimum x coord of LR
...
#define RE 220.f //54.f//2000.f//100.f;
#define UMAX 0.06f
...
{
    int value = 0;
    if(abs(x-OBSTX2) < OBSTR2 && abs(y-OBSTY2) < OBSTR2 && t < 50000)
        value = 10;
...
*/
__global__ void InterpCF(float* f_f, float* g_f, float* h_f, size_t pitch_f, float* m_f_c, float* m_g_c, float* m_h_c, float* m_g_temp, size_t pitch_m, float* f_c, float* g_c, float* h_c, float* g_temp, size_t pitch_c, float SF, float omega_c, int GPU, int zInner, int zInner_f)
{
    ...
    {
        //use g and g_temp
        int x_c = LRX0+threadIdx.x+blockIdx.x*BLOCKSIZEINTERP*LRFACTOR;//in coarse grid, blockdim.x is LRX*LRFACTOR
        int y_c = LRY0+threadIdx.y+blockIdx.y;//in coarse grid, blockdim.y is 1
        int ymax = YLRDIM*LRFACTOR+1;
        float f[19];
        if(threadIdx.z == 0){
        for(int i = 0; i<19; i++)
            f[i]= g_temp[buff_mem(i,x_c,y_c,pitch_c)];
        Moments(f,mom_c[threadIdx.x][threadIdx.y][threadIdx.z]);
        for(int i = 0; i<19; i++)
...
int main(int argc, char *argv[])
{
    ...
    for(int t = 0; t<TMAX; t++)
    {
        ...
        for(int n = 0; n<GPU_N; n++)
            cudaMemcpyPeerAsync(&h_temp[n][pitch_e*YDIM],n,&g_d[   (n+1)%GPU_N][A][pitch_e*YDIM],   (n+1)%GPU_N,pitch_e*YDIM*sizeof(float)*19,stream_halo[n]);
        for(int n = 0; n<GPU_N; n++)
            cudaMemcpyPeerAsync(&g_temp[n][pitch_e*YDIM],n,&h_d[abs(n-1)%GPU_N][A][pitch_e*YDIM],abs(n-1)%GPU_N,pitch_e*YDIM*sizeof(float)*19,stream_halo[n]);
...
//            dpdy *= 0.99f;
        ...
        for(int n = 0; n<GPU_N; n++){
        cudaSetDevice(n);
        InterpCF<<<Interp_grid,Interp_threads,0,stream_inner[n]>>>(f_LR_d[n][C],g_LR_d[n][C],h_LR_d[n][C],pitchLR_e,f_interp[n],g_interp[n],h_interp[n],g_interp_temp[n],pitchInterp_e,f_d[n][A],g_d[n][A],h_d[n][A],g_temp[n],pitch_e,SF_cf,omega,n,zInner,zLRInner);
        //cudaDeviceSynchronize();
        }
