...
#define LRX0 111.0078125f	//minimum x coord of LR
...
inline __device__ int ImageFcn(int x, int y){
	...
	if(x < 0.1f)
		...
		return 11;//xsymmetry top
	else if(y < 0.1f)
		return 12;//xsymmetry bottom
	//else if((x-OBSTX)*(x-OBSTX)+(y-OBSTY)*(y-OBSTY)<OBSTR*OBSTR)
	if(abs(x-OBSTX)<OBSTR && abs(y-OBSTY)<OBSTR)
		return 10;
	else
...
//	usqr = u*u+v*v;
	...
	f[j+0 *pitch*YDIM]= x*1.000001f;//0.4444444444f*(rho-1.5f*usqr);
	f[j+1 *pitch*YDIM]= x*1.000001f;//0.1111111111f*(rho+3.0f*u+4.5f*u*u-1.5f*usqr);               
	f[j+2 *pitch*YDIM]= x*1.000001f;//0.1111111111f*(rho+3.0f*v+4.5f*v*v-1.5f*usqr);
	f[j+3 *pitch*YDIM]= x*1.000001f;//0.1111111111f*(rho-3.0f*u+4.5f*u*u-1.5f*usqr);
	f[j+4 *pitch*YDIM]= x*1.000001f;//0.1111111111f*(rho-3.0f*v+4.5f*v*v-1.5f*usqr);
	f[j+5 *pitch*YDIM]= x*1.000001f;//0.02777777778*(rho+3.0f*( u+v)+4.5f*( u+v)*( u+v)-1.5f*usqr);
	f[j+6 *pitch*YDIM]= x*1.000001f;//0.02777777778*(rho+3.0f*(-u+v)+4.5f*(-u+v)*(-u+v)-1.5f*usqr);
	f[j+7 *pitch*YDIM]= x*1.000001f;//0.02777777778*(rho+3.0f*(-u-v)+4.5f*(-u-v)*(-u-v)-1.5f*usqr);
	f[j+8 *pitch*YDIM]= x*1.000001f;//0.02777777778*(rho+3.0f*( u-v)+4.5f*( u-v)*( u-v)-1.5f*usqr);
	}
}
...
//	usqr = u*u+v*v;
	...
	f[j+0 *pitch*YLRDIM]= tex2D(texRef_f0A,xcoord+0.5f,ycoord+0.5f);//0.4444444444f*(rho-1.5f*usqr);
	f[j+1 *pitch*YLRDIM]= tex2D(texRef_f1A,xcoord+0.5f,ycoord+0.5f);//0.1111111111f*(rho+3.0f*u+4.5f*u*u-1.5f*usqr);               
	f[j+2 *pitch*YLRDIM]= tex2D(texRef_f2A,xcoord+0.5f,ycoord+0.5f);//0.1111111111f*(rho+3.0f*v+4.5f*v*v-1.5f*usqr);
	f[j+3 *pitch*YLRDIM]= tex2D(texRef_f3A,xcoord+0.5f,ycoord+0.5f);//0.1111111111f*(rho-3.0f*u+4.5f*u*u-1.5f*usqr);
	f[j+4 *pitch*YLRDIM]= tex2D(texRef_f4A,xcoord+0.5f,ycoord+0.5f);//0.1111111111f*(rho-3.0f*v+4.5f*v*v-1.5f*usqr);
	f[j+5 *pitch*YLRDIM]= tex2D(texRef_f5A,xcoord+0.5f,ycoord+0.5f);//0.02777777778*(rho+3.0f*( u+v)+4.5f*( u+v)*( u+v)-1.5f*usqr);
	f[j+6 *pitch*YLRDIM]= tex2D(texRef_f6A,xcoord+0.5f,ycoord+0.5f);//0.02777777778*(rho+3.0f*(-u+v)+4.5f*(-u+v)*(-u+v)-1.5f*usqr);
	f[j+7 *pitch*YLRDIM]= tex2D(texRef_f7A,xcoord+0.5f,ycoord+0.5f);//0.02777777778*(rho+3.0f*(-u-v)+4.5f*(-u-v)*(-u-v)-1.5f*usqr);
	f[j+8 *pitch*YLRDIM]= tex2D(texRef_f8A,xcoord+0.5f,ycoord+0.5f);//0.02777777778*(rho+3.0f*( u-v)+4.5f*( u-v)*( u-v)-1.5f*usqr);
	}
}
...
//	float SF_fc3 = 1.f/SF_cf3;
    ...
    dim3 grid(XDIM/BLOCKSIZEX+XDIM%BLOCKSIZEX,YDIM/BLOCKSIZEY+YDIM%BLOCKSIZEY);
...
//			<<double((XDIM*YDIM*ZDIM)*double(TMAX/1000000.f))/restime<<"MLUPS)"<<endl;
	...
	for(row = 0; row<YDIM; row++){
		for(col = 0; col<XDIM; col++){
			...
			output<<col<<", "<<row<<", "<<u<<","<<v<<","<<fA_h[i]<<endl;
		}
	}
	...
	for(row = LRLEVEL; row<YLRDIM-LRLEVEL; row++){
		for(col = LRLEVEL; col<XLRDIM-LRLEVEL; col++){
			...
			output<<LRX0+LRFACTOR*col<<", "<<LRY0+LRFACTOR*row<<", "<<u<<","<<v<<","<<fC_h[i]<<endl;
		}
	}
	}
	...
	cout<<fA_h[175*XDIM+112]<<","<<fA_h[175*XDIM+113]<<","<<fA_h[176*XDIM+112]<<","<<fA_h[176*XDIM+113]<<endl;
	cout<<(3.f*fA_h[175*XDIM+112]+9.f*fA_h[175*XDIM+113]+fA_h[176*XDIM+112]+3.f*fA_h[176*XDIM+113])/16.f<<endl;
	cout<<setprecision(32)<<(1.f-(LRX0-int(LRX0)))*fA_h[175*XDIM+112]+(LRX0-int(LRX0))*fA_h[175*XDIM+113]<<endl;
	cout<<setprecision(32)<<fC_h[(YLRDIM-LRLEVEL-1)*XLRDIM+LRLEVEL]<<endl;
